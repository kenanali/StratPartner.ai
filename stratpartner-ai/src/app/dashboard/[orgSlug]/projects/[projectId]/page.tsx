  ...
  return (
    <div className="p-8">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        {/* Tasks */}
        <div>
          <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Tasks</h2>
          <div className="space-y-2">
            {tasks?.map((task) => (
              <div key={task.id} className="rounded-xl border border-gray-200 bg-white px-4 py-3">
        ...
        {/* Deliverables */}
        <div>
          <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Deliverables</h2>
          <div className="space-y-2">
            {deliverables?.map((d) => (
              <Link
                key={d.id}
                href={`/dashboard/${params.orgSlug}/projects/${project.id}/deliverables/${d.id}`}
                className="flex items-center justify-between rounded-xl border border-gray-200 bg-white px-4 py-3 hover:border-violet-300 hover:shadow-sm transition-all"
              >
                <div className="min-w-0">
        ...
        {project.memory && (
          <div className="lg:col-span-2">
            <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Project Context</h2>
